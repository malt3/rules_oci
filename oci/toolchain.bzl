...
registry_toolchain = rule(
    ...
    attrs = {
        "launcher": attr.label(
            doc = "A bash launcher script defining a bash function named `start_registry` that takes the following arguments `storage_dir, output, deadline`; `get_registry` returns the registry address; `stop_registry` stops the registry.",
            mandatory = True,
            executable = True,
            cfg = "exec",
            allow_single_file = True,
